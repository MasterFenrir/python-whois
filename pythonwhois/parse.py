...
registrant_fallback_regexes = {
    "handle": "Registrant[ \t\S]*ID:(?P<handle>.+)",
    "name": "Registrant[ \t\S]*Name:(?P<name>.*)",
    "organization": "Registrant[ \t\S]*Organization:(?P<organization>.*)",
    "street": "Registrant[ \t\S]*Street:(?P<street>.*)",
    "city": "Registrant[ \t\S]*City:(?P<city>.*)",
    "state": "Registrant[ \t\S]*State:(?P<state>.*)",
    "postalcode": "Registrant[ \t\S]*Postal Code:(?P<postalcode>.*)",
    "country": "Registrant[ \t\S]*Country:(?P<country>.*)",
    "phone": "Registrant[ \t\S]*Phone:(?P<phone>.*)",
    "fax": "Registrant[ \t\S]*Fax:(?P<fax>.*)",
    "email": "Registrant[ \t\S]*Email:(?P<email>.*)"
}

admin_fallback_regexes = {
    "handle": "Admin[ \t\S]*ID:(?P<handle>.+)",
    "name": "Admin[ \t\S]*Name:(?P<name>.*)",
    "organization": "Admin[ \t\S]*Organization:(?P<organization>.*)",
    "street": "Admin[ \t\S]*Street:(?P<street>.*)",
    "city": "Admin[ \t\S]*City:(?P<city>.*)",
    "state": "Admin[ \t\S]*State:(?P<state>.*)",
    "postalcode": "Admin[ \t\S]*Postal Code:(?P<postalcode>.*)",
    "country": "Admin[ \t\S]*Country:(?P<country>.*)",
    "phone": "Admin[ \t\S]*Phone:(?P<phone>.*)",
    "fax": "Admin[ \t\S]*Fax:(?P<fax>.*)",
    "email": "Admin[ \t\S]*Email:(?P<email>.*)"
}

billing_fallback_regexes = {
    "handle": "Billing[ \t\S]*ID:(?P<handle>.+)",
    "name": "Billing[ \t\S]*Name:(?P<name>.*)",
    "organization": "Billing[ \t\S]*Organization:(?P<organization>.*)",
    "street": "Billing[ \t\S]*Street:(?P<street>.*)",
    "city": "Billing[ \t\S]*City:(?P<city>.*)",
    "state": "Billing[ \t\S]*State:(?P<state>.*)",
    "postalcode": "Billing[ \t\S]*Postal Code:(?P<postalcode>.*)",
    "country": "Billing[ \t\S]*Country:(?P<country>.*)",
    "phone": "Billing[ \t\S]*Phone:(?P<phone>.*)",
    "fax": "Billing[ \t\S]*Fax:(?P<fax>.*)",
    "email": "Billing[ \t\S]*Email:(?P<email>.*)"
}

tech_fallback_regexes = {
    "handle": "Tech[ \t\S]*ID:(?P<handle>.+)",
    "name": "Tech[ \t\S]*Name:(?P<name>.*)",
    "organization": "Tech[ \t\S]*Organization:(?P<organization>.*)",
    "street": "Tech[ \t\S]*Street:(?P<street>.*)",
    "city": "Tech[ \t\S]*City:(?P<city>.*)",
    "state": "Tech[ \t\S]*State:(?P<state>.*)",
    "postalcode": "Tech[ \t\S]*Postal Code:(?P<postalcode>.*)",
    "country": "Tech[ \t\S]*Country:(?P<country>.*)",
    "phone": "Tech[ \t\S]*Phone:(?P<phone>.*)",
    "fax": "Tech[ \t\S]*Fax:(?P<fax>.*)",
    "email": "Tech[ \t\S]*Email:(?P<email>.*)"
}
...
def fallback_extraction(data, contact, regexes):
    """
    Try to use the fallback rules to extract any missed data
    :param data:  The array containing the data
    :param contact: The contact to check
    :param regexes: The regexes to use
    :return: The new contact if the result is not None, otherwise None
    """
    if contact is None:
        temp_contact = {}
    else:
    ...
    for segment in data:
        get_contact_field_from_response(segment, "handle", temp_contact, regexes)
        get_contact_field_from_response(segment, "name", temp_contact, regexes)
        get_contact_field_from_response(segment, "organization", temp_contact, regexes)
        get_contact_field_from_response(segment, "street", temp_contact, regexes)
        get_contact_field_from_response(segment, "city", temp_contact, regexes)
        get_contact_field_from_response(segment, "state", temp_contact, regexes)
        get_contact_field_from_response(segment, "postalcode", temp_contact, regexes)
        get_contact_field_from_response(segment, "country", temp_contact, regexes)
        get_contact_field_from_response(segment, "phone", temp_contact, regexes)
        get_contact_field_from_response(segment, "fax", temp_contact, regexes)
        get_contact_field_from_response(segment, "email", temp_contact, regexes)
...
def get_contact_field_from_response(segment, field, contact, regexes):
    """
    Check if a contact contains a given field and if it does not,
    see if it can be extracted from the WHOIS response
    :param segment: The WHOIS response to check
    :param field: The field to check for
    :param contact: The contact to verify
    :param regexes: The set of regexes to use
    :return Does not return anything, but modifies the contact
    """
    if field not in contact:
        match = re.search(regexes.get(field), segment, re.IGNORECASE)
        if match is not None:
...
def parse_registrants(data, never_query_handles=True, handle_server=""):
    ...
    registrant = fallback_extraction(data, registrant, registrant_fallback_regexes)
    tech_contact = fallback_extraction(data, tech_contact, tech_fallback_regexes)
    admin_contact = fallback_extraction(data, admin_contact, admin_fallback_regexes)
    billing_contact = fallback_extraction(data, billing_contact, billing_fallback_regexes)
